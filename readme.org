...
- Add/Remove buffer into/from project if the file is not in the project


** Will not implement
- Window layout restoration
  - This is one of the most difficult things in session management. Other packages have been trying to manage this many years, but it seems subjective - everyone has different preferences
- Customization of the desktop directory name
  - The =.emacs.desktop= file is not portable between machines.
  - If you change the project's location, it becames incompatible and cannot be reused.
...
** Incompatibility of desktop.el's own `desktop.el`
We use original functions as possible in `projab`, but some functionally needs to be omitted.

- `projab` cannot serialize/deserialize buffer variable
  - This means that restored buffer cannot have any history of mark ring, anyway
